...
int StaticPointToStaticRect(Vector2D *pPos, Vector2D *pRect, float Width, float Height)
{
	...
	float _right = pRect->x + Width / 2;
	float _left = pRect->x - Width / 2;
	float _top = pRect->y + Height / 2;
	float _bottom = pRect->y - Height / 2;
...
int StaticRectToStaticRect(Vector2D *pRect0, float Width0, float Height0, Vector2D *pRect1, float Width1, float Height1)
{
	...
	float r0_left = pRect0->x - Width0 / 2;
	float r0_right = pRect0->x + Width0 / 2;
	float r0_top = pRect0->y + Height0 / 2;
	float r0_bottom = pRect0->y - Height0 / 2;

	float r1_right = pRect1->x + Width1 / 2;
	float r1_left = pRect1->x - Width1 / 2;
	float r1_top = pRect1->y + Height1 / 2;
	float r1_bottom = pRect1->y - Height1 / 2;
...
float StaticPointToStaticLineSegment(Vector2D *P, LineSegment2D *LS)
{
	...
	return Vector2DDotProduct(&LS->mN, &edge);
}
...
float AnimatedPointToStaticLineSegment(Vector2D *Ps, Vector2D *Pe, LineSegment2D *LS, Vector2D *Pi)
{
	...
	dot_with_ps = Vector2DDotProduct(Ps, &LS->mN);
	dot_with_pe = Vector2DDotProduct(Pe, &LS->mN);
	dot_with_vel = Vector2DDotProduct(&point_vel, &LS->mN);


	if (dot_with_pe < LS->mNdotP0 && dot_with_ps < LS->mNdotP0) {

		return -1.0f;
	}
	else if (dot_with_pe > LS->mNdotP0 && dot_with_ps > LS->mNdotP0) {
	...
	time_of_interesection = (LS->mNdotP0 - dot_with_ps) / dot_with_vel;
	...
	Vector2DSub(&intersection_to_po, Pi, &LS->mP0);
	Vector2DSub(&po_to_p1, &LS->mP1, &LS->mP0);
	...
	Vector2DSub(&intersection_to_po, Pi, &LS->mP1);
	Vector2DSub(&po_to_p1, &LS->mP0, &LS->mP1);
...
float AnimatedCircleToStaticLineSegment(Vector2D *Ps, Vector2D *Pe, float Radius, LineSegment2D *LS, Vector2D *Pi)
{
	...
	if (dot_with_pe - LS->mNdotP0 < -Radius && dot_with_ps - LS->mNdotP0 < -Radius) {

		return -1.0f;
	}
	else if (dot_with_pe - LS->mNdotP0 > Radius && dot_with_ps - LS->mNdotP0 > Radius) {
	...
	time_of_interesection = (LS->mNdotP0 - dot_with_ps + Radius) / dot_with_vel;
...
float ReflectAnimatedPointOnStaticLineSegment(Vector2D *Ps, Vector2D *Pe, LineSegment2D *LS, Vector2D *Pi, Vector2D *R)
{
	...
	dot_i_to_n = Vector2DDotProduct(&pi_to_pe, &LS->mN);

	//s
	Vector2DScale(&normal, &LS->mN, dot_i_to_n * 2);
